    ...
    if re.search(r'@olam\.com|@olamagri\.com', corpo_email):
        ...
        if message.attachments:
            for attachment in message.attachments:
                ...
                if file_extension in ".zip" or file_extension in ".pdf" or file_extension in '.xlsx' or file_extension in '.xlsm':
                    ...
                    for attachment in message.attachments:
                            ...
                            try:
                                with pd.ExcelFile(temp_xlsx.name, engine='openpyxl') as excel_file:
                                    ...
                                    if "NOTAS ORIGEM" in sheet_names:
                                        ...
                                        print('notas origem')
                                        nf_ref_cleaned = df['Nº Nota Fiscal'].astype(str)
                                        ...
                                        nota_comp = df2['Nº Nota Fiscal'].astype(str)
                                        ...
                                        serie = df2['Série'].dropna().astype(str)
                                        dta_emissao = df2['Data Emisão'].dropna().astype(str)
                                        ...
                                        nf_comp = df['Nº Nota Fiscal'].astype(str)
                                        serie = df['Série']
                                        ...
                                        nf_excel_map[file_name] = {'nota_fiscal': '0',
                                                                   'data_email': message.received,
                        ...
                        if file_extension == '.xlsm':
                            with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as temp_xlsx:
                                ...
                                if 'NF' in df.columns and 'Chave de acesso' in df.columns:
                                    ...
                                    for nf, chave, nfe_serie, peso_nfe, nota, nota_comp, cnpj in zip(nf_ref_cleaned,
                                        ...
                                        for match in nota:
                                            chaves = ''.join(chave)
                                            nf_excel_map[match] = {'nota_fiscal': match,
                                                                   'data_email': message.received,
                                                                   'chave_acesso': '0',
                                                                   'email_vinculado': message.subject,
                                                                   'serie_nf': nfe_serie,
                            ...
                            pass #print("Anexo ignorado ou tipo não suportado.")
